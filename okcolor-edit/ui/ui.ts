...
const curveEditor = byId<HTMLCanvasElement>("curveEditor");
...
function renderCurveEditor(activeChannel: "l" | "a" | "b" | null = null): void {
  ...
  const handleY = (value: number) => height - pad - value * (height - pad * 2);
...
let draggingChannel: "l" | "a" | "b" | null = null;

function findCurveChannelByY(y: number): "l" | "a" | "b" {
  const mids = [
    { id: "l" as const, value: Number(curveMid.value) },
    { id: "a" as const, value: Number(curveMidA.value) },
    { id: "b" as const, value: Number(curveMidB.value) }
  ];
  const pad = 10;
  const h = curveEditor.height;
  const targetValue = (h - pad - y) / Math.max(1, h - pad * 2);
  return mids
    .sort((left, right) => Math.abs(left.value - targetValue) - Math.abs(right.value - targetValue))[0]
    .id;
}

function updateCurveMidpointFromPointer(evt: PointerEvent): void {
...
const stopCurveDrag = (evt: PointerEvent) => {
  if (!draggingChannel) return;
